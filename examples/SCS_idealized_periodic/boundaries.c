...
void BoundaryVelocities(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm) {
  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
    ...
    // if( (k_hatx*rn1+k_haty*rn2) < 0){   // Flux dot rnormal is negative = incoming wave
    //   k_hat=1;
    // }else{
    //   k_hat=0;
    // }
    //printf("proc=%d, edge=%d, j=%d, xe=%f, k_hat=%f,rn1=%f, rn2=%f,\n",myproc,jind,j,grid->xe[j],k_hat,rn1,rn2 );
    z=0;
    for(k=grid->etop[j];k<grid->Nke[j];k++) {
      ...
      // u_LS_bar = ULm + ULz*fz;
      // u_LS_prime =  UHtide * cos(ktide*xe - 2*PI*prop->rtime/TM2 + Phitide) +
      //       k_hat * UHiw * fz * cos(kiw*xe - 2*PI*prop->rtime/TM2 + Phiiw);
      // v_LS_bar=0;
      // v_LS_prime=0;
