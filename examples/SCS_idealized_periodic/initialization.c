...
REAL ReturnSalinity(REAL x, REAL y, REAL z, propT *prop) {
  REAL deltaS, alphaS, D_pycnocline, rho;
  REAL delta=prop->delta, drho=prop->drho, h2=prop->h2;
  REAL D=prop->D;


  // use this with beta=1e-3, specify rho directly
  //rho=1;
  //rho = drho/2*(1-tanh((z+D-h2)/delta));
  return 0;
}
...
REAL ReturnTemperature(REAL x, REAL y, REAL z, REAL depth) {
  REAL alpha=1e-4, g = 9.81, dT, c0=0.25, Fr;
    REAL U=0.88, N=0.007, D = 6911 +0*16000, Lx = 5000, h=200, xc=145800+0*186000;

  dT = N*N*D/alpha/g;
  REAL a1 = 23.3637651923536;
  REAL a2 = 3.18189680534772;
  REAL a3 = -44.1196509264888;
  REAL a4 = 293.118606564064;

  REAL temp = a1*exp(-(-z+a3)/a4)+a2;

  // uniform stratigfication
  return temp; /*20+z*dT/D;*/
...
REAL ReturnHorizontalVelocity(REAL x, REAL y, REAL n1, REAL n2, REAL z) {
  REAL U=0, N=0.007, D = 6911 +0*16000, Lx = 5000, h=200, xc=145800+0*186000;

  // depth averaged uniform flow
  //return 0; /*U*D/ReturnDepth(x,y)*n1;*/

  // queney's solution
  // return (U*D/ReturnDepth(x,y) + N*h*Lx/(Lx*Lx + (x-xc)*(x-xc))*(Lx*sin(N*(z+D+h)/U) + (x-xc)*cos(N*(z+D+h)/U)))*n1;

  return U*n1; // periodic forcing via 3D coriolis
  // return 0;
}
