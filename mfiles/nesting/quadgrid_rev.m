...
%
% revised Justin Rogers 7/2017
% 1) developed function input
% 2) fixed error in type2 B/C on West and North sides
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [] = quadgrid_rev(datadir,L,W,Nx,Ny,BC,STRETCHING,CHEBYCHEV,Lr,K,rmax)
% datadir='../rundata Directory in which points.dat, cells,dat, edges.dat files will be
% L = 200E3; Length of domain
% W = 1.732E3; Width of domain
% Nx = L/2000; Number of cells in x
% Ny = 3; Number of cells in y
% BC = [1 1 2 1]; E,N,W,S Boundary conditions
%                   Boundary condition types:
%                   1 solid free-slip wall
%                   2 velocity specified
%                   3 free-surface specified
% STRETCHING='false', whether to employ stretching
% CHEBYCHEV='false' use Chebechev grids
% Lr= 20; length of resolved region
% K=2; the resolution for dx/dxr
% rmax = 1.1, Maximum acceptable stretching factor.
...
% J Rogers 2017
% boundary should always be last column (-1)
for i=1:size(edgeoutput,1)
   if edgeoutput(i,4)==-1
       edgeoutput(i,4)=edgeoutput(i,5);
       edgeoutput(i,5)=-1;
   end
end
...
save('SUNTANS_grid.mat','-append','xv','yv','points','edges');
...
% figure(2342)
% 
% plot(xv,yv,'xk') %voronoi centers
% hold on
% 
% for i=1:size(edges,1) % suntans uses C indexing starts at 0
...
%     
% end
% for i =1:size(pointoutput,1)
%     txt{i} = num2str(i-1);
% end
% text(pointoutput(:,1)+10,pointoutput(:,2),txt)
% clear txt
% for i =1:size(celloutput,1)
%     txt{i} = num2str(i-1);
% end
% text(celloutput(:,2)+10,celloutput(:,3),txt)
% 
